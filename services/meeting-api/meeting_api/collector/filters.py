...
class TranscriptionFilter:
    ...
    def load_config(self):
        """Load filter configuration from filter_config module."""
        try:
            ...
            logger.warning("No filter_config module found, using default settings")
    ...
    def filter_segment(self, text: str, start_time: float, end_time: float, meeting_id: int, language: str ='en', speaker: str = None):
        """
        ...
        Args:
            ...
            speaker (str): Speaker name — dedup only applies within the same speaker
        ...
        for i, cached_segment in enumerate(current_meeting_cache):
            # Only dedup within the same speaker — different speakers can talk
            # at the same time and produce overlapping time ranges legitimately
            if speaker and cached_segment.get('speaker') and speaker != cached_segment['speaker']:
                continue
            ...
            if text == cached_text:
                ...
                if start_time >= cached_start and end_time <= cached_end:
                    logger.debug(f"Filtering segment (identical text, sub-segment/duplicate): MeetingID {meeting_id}, speaker '{speaker}', '{text}' ({start_time}-{end_time}) due to cached: '{cached_text}' ({cached_start}-{cached_end})")
                ...
                # Case 1b: Cached is sub-segment of current (current is expansion) -> mark cached for removal
                    ...
                    logger.debug(f"Current segment (identical text, expansion): MeetingID {meeting_id}, speaker '{speaker}', '{text}' ({start_time}-{end_time}). Marking cached sub-segment for removal: '{cached_text}' ({cached_start}-{cached_end})")
                    indices_to_remove_from_cache.append(i)

            # Condition 2: Text is different, but significant temporal overlap (same speaker only).
            else:
                ...
                if max(start_time, cached_start) < min(end_time, cached_end) and current_duration > min_duration_for_diff_text_overlap_check and cached_duration > min_duration_for_diff_text_overlap_check:
                    ...
                    if start_time >= cached_start and end_time <= cached_end and cached_duration > current_duration and len(text) < len(cached_text):
                        logger.debug(f"Filtering segment (different text, shorter, sub-segment of longer cached): MeetingID {meeting_id}, speaker '{speaker}', '{text}' ({start_time}-{end_time}) due to cached: '{cached_text}' ({cached_start}-{cached_end})")
                        should_filter_current = True
                        break
                    ...
                    # Case 2b: Cached segment is fully temporally contained within a longer current segment.
                        ...
                        logger.debug(f"Current segment (different text, longer, expansion over cached): MeetingID {meeting_id}, speaker '{speaker}', '{text}' ({start_time}-{end_time}). Marking shorter cached for removal: '{cached_text}' ({cached_start}-{cached_end})")
                        indices_to_remove_from_cache.append(i)
        ...
        current_meeting_cache.append({'text': text, 'start': start_time, 'end': end_time, 'speaker': speaker})
        return True
