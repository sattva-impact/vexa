...
Set `STORAGE_BACKEND` in `meeting-api`:
...
`deploy/compose/docker-compose.yml` mounts a persistent named volume:
...
Use local backend with a PVC mounted into the meeting-api pod:
...
Use `minio` (self-hosted) or `s3` (managed cloud) and keep meeting-api stateless.
