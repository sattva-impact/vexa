...
<Warning>
**Zoom SDK integration is for self-hosted (open source) deployments only.** The hosted service at vexa.ai does not yet support Zoom. A browser-based approach (no SDK needed) is in development.
</Warning>

Use this guide to configure Zoom for the Vexa meeting bot in self-hosted deployments.
...
```text
Zoom Meeting SDK -> PCM audio -> transcription-service -> transcript output
```
...
**Zoom is not available on the hosted service (vexa.ai).** This setup guide applies to self-hosted (open source) deployments only. A browser-based Zoom web app (no SDK needed) is in development and will enable Zoom on the hosted service when released.
...
Required local SDK artifacts (self-hosted):
...
services/vexa-bot/core/src/platforms/zoom/native/zoom_meeting_sdk/qt_libs/... (from Zoom SDK package)
```

The Zoom Meeting SDK binaries are proprietary and are not included in this repo. Download them from Zoom and place them at the paths above before running Zoom bots.

If these files are missing, bot creation may return `201` initially, but the meeting quickly transitions `joining` -> `failed` with an `error_details` message containing `Zoom SDK native addon is not available` (tracked in [#128](https://github.com/Vexa-ai/vexa/issues/128)).

Implementation detail:

- Dashboard and meeting-api resolve OAuth credentials with fallback:
...
- `vexa/services/meeting-api/meeting_api/meetings.py`
- `vexa/services/meeting-api/meeting_api/zoom_obf.py`
...
- `services/dashboard/src/app/api/zoom/oauth/start/route.ts`
- `services/dashboard/src/app/api/zoom/oauth/complete/route.ts`
