...
<Info>
`POST /bots` returns `201 Created` with the meeting record. The bot container is launched in the background. Use `GET /bots/status` or WebSocket `meeting.status` events to track the bot's progress.
</Info>
