# Release validation report — `0.10.0-260419-0052`

_Generated 2026-04-18T22:43:10.329504Z from `tests3/.state/reports/`._

## Scope status

**Release**: `260418-webhooks` — Webhook status-event coverage. Non-`meeting.completed` events

| Issue | Required modes | Status per proof | Verdict |
|-------|----------------|-------------------|---------|
| `webhooks-status-events-non-completed` | compose | compose `webhooks/e2e_status_non_completed`: ✅ pass<br>compose `webhooks/e2e_status`: ✅ pass | **✅ pass** |
...
| `bot-lifecycle` | **75%** | 90% | ❌ below gate |
...
| status-webhooks-fire | Status-change webhooks fire for every transition when enabled in webhook_events | 5 | ✅ pass | compose: webhooks/e2e_status: 4 status-change webhook(s) fired: meeting.status_change |
